...
validate_registry() {
    if [ -z "$REGISTRY" ]; then
        echo -e "${YELLOW}No registry specified. Please provide an ABAC-enabled registry.${NC}"
        echo "Usage: $0 <registry> [test_mode]"
        echo "Example: $0 myregistry.azurecr.io comprehensive"
        exit 1
    fi
    
    echo -e "${CYAN}Validating registry: $REGISTRY${NC}"
    ...
    if ! az acr show --name "$registry_name" >/dev/null 2>&1; then
        echo -e "${RED}Error: Registry '$registry_name' not found or not accessible.${NC}"
        exit 1
    fi
